...
case class KeyScheduleInputPort(key_width: Int) extends Bundle {
  val key      = Vec.fill(key_width/8)(UInt(8 bits))
  val constant = UInt(8 bits)
}

case class KeyScheduleOutputPort(key_width: Int) extends Bundle {
  val key      = Vec.fill(key_width/8)(UInt(8 bits))
}
...
case class KeySchedule(key_width: Int, dual_ported: Boolean = true) extends Component {
  ...
  val io = new Bundle {
    val source      =  slave(Stream(KeyScheduleInputPort(key_width)))
    val destination = master(Stream(KeyScheduleOutputPort(key_width)))
  }
  ...
  // Reshape key
  val key_by_32 = io.source.payload.key.asBits.subdivideIn(32 bits)
  // To result
  val round_key_by_32 = Vec.fill(key_width/32)(Bits(32 bits))
  // Handle word 3
  ...
  // Result
  //  TODO: can it be replaced by `io.destination.payload.key := round_key_by_32.asBits.subdivideIn(8 bits)` ?
  val res = round_key_by_32.asBits.subdivideIn(8 bits)
  // Cast to vec of UInts
  for (i <- 0 until widthOf(res)/8)
    io.destination.payload.key(i) := res(i).asUInt
}
